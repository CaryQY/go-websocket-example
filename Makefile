...
gox-linux:
	CGO_ENABLED=0 gox -ldflags '-s -w -X "main.mode=prod"' -osarch="linux/amd64 linux/arm64" -output="build/go-websocket-example_{{.OS}}_{{.Arch}}"

gox-all:
	CGO_ENABLED=0 gox -ldflags '-s -w -X "main.mode=prod"' -osarch="darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64" -output="build/go-websocket-example_{{.OS}}_{{.Arch}}"
